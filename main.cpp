...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    bool ignoreBluetooth = false;
    int V = 127, W = 127;
    
    if (irValues[0] < BRANCO) { // Esquerda frente
      ignoreBluetooth = true;
      V = 0;
      W = 254;
    }
    if (irValues[1] < BRANCO) { // Direita frente
      ignoreBluetooth = true;
      V = 254;
      W = 254;
    }
    if (irValues[3] < BRANCO) { // Esquerda trás
      ignoreBluetooth = true;
      V = 0;
      W = 0;
    }
    if (irValues[2] < BRANCO) { // Direita trás
      ignoreBluetooth = true;
      V = 254;
      W = 0;
    }

    if (irValues[0] < BRANCO || irValues[1] < BRANCO) { // Esquerda frente e Direita frente
      ignoreBluetooth = true;
      V = 0;
      W = 127;
    }
